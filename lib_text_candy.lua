...
COPYRIGHT:		(C)2011,2012,2013 MIKE DOGAN GAMES & ENTERTAINMENT
...
local SetCharXOffset = function (fontName, char, offX)
...
local AddCharset = function (name, dataFile, imageFile, charOrder, spaceWidth, charSpacing, lineSpacing)
	...
	V.Charsets[name] = {}
	V.Charsets[name].name        = name
	V.Charsets[name].isVector    = false
	V.Charsets[name].dataFile    = dataFile
	V.Charsets[name].imageFile   = imageFile
	V.Charsets[name].Data        = Data
	V.Charsets[name].charString  = charOrder
	V.Charsets[name].numChars    = string.len(charOrder)
	V.Charsets[name].Chars       = {}
	V.Charsets[name].lineHeight  = 0
	V.Charsets[name].spaceWidth  = spaceWidth
	V.Charsets[name].charSpacing = charSpacing
	V.Charsets[name].lineSpacing = lineSpacing
	V.Charsets[name].scale       = 1.0
	...
	for i = 1, string.len(charOrder) do
		...
		n = string.byte(c)
		V.Charsets[name].Chars[n]         = {}
		V.Charsets[name].Chars[n].frame   = i
		V.Charsets[name].Chars[n].offY    = 0
		V.Charsets[name].Chars[n].width   = V.Charsets[name].Data.frames[i].spriteSourceSize.width
		V.Charsets[name].Chars[n].height  = V.Charsets[name].Data.frames[i].spriteSourceSize.height
		V.Charsets[name].Chars[n].xadvance= V.Charsets[name].Chars[n].width
		V.Charsets[name].Chars[n].imgName = V.Charsets[name].Data.frames[i].name

		-- GET HEIGHT OF TALLEST CHAR
		if V.Charsets[name].Chars[n].height > V.Charsets[name].lineHeight then V.Charsets[name].lineHeight = V.Charsets[name].Chars[n].height end
	
		-- GET FRAME DATA FOR IMAGE SHEET
		table.insert(sheetFrames, 
			{
			x      = V.Charsets[name].Data.frames[i].textureRect.x,
			y      = V.Charsets[name].Data.frames[i].textureRect.y,
			width  = V.Charsets[name].Data.frames[i].textureRect.width,
			height = V.Charsets[name].Data.frames[i].textureRect.height,
			})
	end

	-- LOAD IMAGE SHEET
	local bmp = display.newImage(imageFile, system.ResourceDirectory, 0,0, true)
	local SheetOptions =
		{
		frames             = sheetFrames,
		sheetContentWidth  = bmp.width,
		sheetContentHeight = bmp.height,
		}
	
	V.Charsets[name].Set = 
		{
		Sheet    = graphics.newImageSheet(imageFile, SheetOptions),
		Sequence = { name = "all", start = 1, count = #sheetFrames },	
		}
	bmp:removeSelf()

	-- SPACE CHAR DATA
	V.Charsets[name].Chars[32] = {}
	V.Charsets[name].Chars[32].frame    = 0
	V.Charsets[name].Chars[32].offY     = 0
	V.Charsets[name].Chars[32].width    = spaceWidth
	V.Charsets[name].Chars[32].height   = V.Charsets[name].lineHeight
	V.Charsets[name].Chars[32].xadvance = V.Charsets[name].Chars[32].width

	-- NEW LINE CHAR DATA
	V.Charsets[name].Chars[13] = {}
	V.Charsets[name].Chars[13].frame    = 0
	V.Charsets[name].Chars[13].offY     = 0
	V.Charsets[name].Chars[13].width    = 0
	V.Charsets[name].Chars[13].height   = 0
	V.Charsets[name].Chars[13].xadvance = V.Charsets[name].Chars[13].width
	
	if V.debug then print ("--> TextFX.AddCharset(): ADDED CHARSET '"..name.."' ("..V.Charsets[name].numChars.." CHARS)") end
end
...
local AddVectorFont = function (fontName, charOrder, fontSize, charSpacing, lineSpacing)
	...
	for i = 1, string.len(charOrder) do
		...
		V.Charsets[fontName].Chars[n].xadvance= Temp.width
	...
	V.Charsets[fontName].Chars[32].xadvance = Temp.width
	...
	V.Charsets[fontName].Chars[13].xadvance = 0
...
----------------------------------------------------------------
local AddCharsetFromBMF = function( name, dataFile, charOrder, spaceWidth )

	local function extract( s, p ) return string.match( s, p ), string.gsub( s, p, '', 1 ) end
	
	-- CHARORDER IS OBSOLETE NOW - KEEP BACKWARD COMPATIBILITY INSTEAD:
	if spaceWidth == nil and type(charOrder) == "number" then spaceWidth = charOrder end 
	...
	for line in readline do
		...
		elseif tag == 'char' then
			...
			if t.letter ~= " " and t.letter ~= "space" then charOrder = charOrder..t.letter end
			Font.chars[ t.letter ] = { }
	...
	-- SPACE CHAR NOT INCLUDED IN DATA FILE?
	    if spaceWidth      ~= nil then Font.chars[' '] = {xadvance = spaceWidth} 
	elseif Font.chars[' '] == nil then Font.chars[' '] = {xadvance = 16}
	end 
	
	AddCharset (name, Font.Sheet, Font.Sheet.file, charOrder, Font.chars[' '].xadvance, 0, Font.info.lineHeight )

	local Charset = V.Charsets[name]
	for i = 1, string.len(charOrder) do
		c   = string.sub (charOrder,i,i)
		num = string.byte(c)
		V.SetCharYOffset( name, c, Font.chars[c].yoffset * .5 )
		Charset.Chars[num].xadvance = Font.chars[c].xadvance
	end
...
----------------------------------------------------------------
-- ADD A LOADED BMF FONT (GLYPH DESIGNER V 2.0)
----------------------------------------------------------------
local AddCharsetFromGlyphDesigner = function( name, dataFile)

	local k,v
	local frame       = 1
	local charOrder   = ""
	local Font        = require(dataFile)
	Font.Sheet        = {}
	Font.Sheet.frames = {}
	local Chars       = {}
	
	for i,v in pairs (Font.chars) do
		Chars[v.id]   = v
		Char          = Chars[v.id]
		if Char.letter ~= " " and Char.letter ~= "space" then charOrder = charOrder..Char.letter end
		if 0 + Char.width > 0 and 0 + Char.height > 0 then
			Font.Sheet.frames[frame] = 
				{
				textureRect = 
					{ 
					x      = 0  + Char.x, 
	...
	-- SPACE CHAR NOT INCLUDED IN DATA FILE?
	    if spaceWidth ~= nil then Chars[32] = {xadvance = spaceWidth} 
	elseif  Chars[32] == nil then Chars[32] = {xadvance = 16} end 
	
	if Font.Sheet.file == nil then Font.Sheet.file = dataFile..".png" end
	
	AddCharset (name, Font.Sheet, Font.Sheet.file, charOrder, Chars[32].xadvance, 0, Font.info.lineHeight )

	local Charset = V.Charsets[name]
	for i = 1, string.len(charOrder) do
		c   = string.sub (charOrder,i,i)
		num = string.byte(c)
		V.SetCharYOffset( name, c, -(Font.common.lineHeight*.5) + Chars[num].yoffset )
		--local yo = -(Font.common.lineHeight - Chars[num].height) + Chars[num].yoffset
		--print (Chars[num].letter.." "..Font.common.lineHeight.." "..Chars[num].height)
		V.SetCharYOffset( name, c, yo )
		Charset.Chars[num].xadvance = Chars[num].xadvance
	end


end
V.AddCharsetFromGlyphDesigner = AddCharsetFromGlyphDesigner
...
local CreateText = function (Properties)
	...
	local Charset = V.Charsets[Properties.fontName]; if Charset == nil then print("!!! TextFX.CreateText(): CHARSET "..Properties.fontName.." NOT FOUND."); return end
	...
	-- SET COLOR -----------------------------------------------
	function Group:setColor(R,G,B,A, R2,G2,B2,A2,D)
		self.Color = {}
		...
		if (R2 and G2 and B2 and A2) then
			...
			self.Color[7] = B2
			self.Color[8] = A2
			self.Color[9] = D  ~= nil and D  or "left"
		end
		local i, Obj
		local n = self[2].numChildren
		for i = 1, n do 
			if self[2][i].isEmptyLine ~= true then
				if self.shadowOffX ~= 0 or self.shadowOffY ~= 0 then Obj = self[2][i][2] else Obj = self[2][i] end
				if V.Charsets[self.fontName].isVector == true then 
					if #self.Color == 9 then Obj:setFillColor( { type='gradient', color1={self.Color[1],self.Color[2],self.Color[3],self.Color[4]}, color2={self.Color[5],self.Color[6],self.Color[7],self.Color[8]}, direction=self.Color[9] } )
							    else Obj:setFillColor( self.Color[1],self.Color[2],self.Color[3],self.Color[4] ) end
				else
					if #self.Color == 9 then Obj:setFillColor( { type='gradient', color1={self.Color[1],self.Color[2],self.Color[3],self.Color[4]}, color2={self.Color[5],self.Color[6],self.Color[7],self.Color[8]}, direction=self.Color[9] } )
							    else Obj:setFillColor( self.Color[1],self.Color[2],self.Color[3],self.Color[4] ) end
				end
	...
	function Group:applyDeform(Properties)
		...
		if self.DeformEffect.flipCharsX == nil then self.DeformEffect.flipCharsX = false end
		if self.DeformEffect.flipCharsY == nil then self.DeformEffect.flipCharsY = false end
		V.PDrawChars (self)
	...
	function Group:addScrollbar( Properties )
		...
		if Properties.FrameTop then
			...
			Properties.FrameTop.anchorX = 0
			Properties.FrameTop.anchorY = 0
		...
		if Properties.Frame then
			...
			Properties.Frame.anchorX = 0
			Properties.Frame.anchorY = 0
		end

		if Properties.FrameBottom then
			Group:insert(Properties.FrameBottom)
			Properties.FrameBottom.anchorX = 0
			Properties.FrameBottom.anchorY = 0
		end
		...
		if Properties.ButtonTop then
			...
			Properties.ButtonTop.anchorX = 0
			Properties.ButtonTop.anchorY = 0
		...
		Properties.Button.anchorX = 0
		Properties.Button.anchorY = 0

		if Properties.ButtonBottom then
			ButtonGroup:insert(Properties.ButtonBottom)
			Properties.ButtonBottom.anchorX	= 0
			Properties.ButtonBottom.anchorY	= 0
		end
...
local GetLineWidth = function (fontName, txt, charSpacing)
	...
	for i = 1, l do
		...
		elseif c ~= V.newLineChar then
			xadvance = Char.xadvance ~= nil and Char.xadvance * Charset.scale or Char.width
			w = w + xadvance
			if i < string.len(txt) then w = w + charSpacing end
		else 
			break
		end
	end
...
local PStartInOutTransition = function(Group)
	...
	if Group.InOutEffect.autoRemoveText == true then 
		Group.InOutTimer = timer.performWithDelay(duration, function() if Group.InOutEffect.CompleteListener ~= nil then Group.InOutEffect.CompleteListener ( { name = "transitionComplete", target = Group } ) end Group:delete() end ) 
	elseif Group.InOutEffect.loop == true then 
		Group.InOutTimer = timer.performWithDelay(duration, function() if Group.InOutEffect.CompleteListener ~= nil then Group.InOutEffect.CompleteListener ( { name = "transitionComplete", target = Group } ) end V.PDrawChars(Group); if Group.InOutEffect.restartOnChange == false then V.PStartInOutTransition(Group) end end )
	else
		Group.InOutTimer = timer.performWithDelay(duration, function() if Group.InOutEffect.CompleteListener ~= nil then Group.InOutEffect.CompleteListener ( { name = "transitionComplete", target = Group } ) end end )
	end
end
...
local PApplyProperties = function(Group, Properties)
	...
	local fontName = Properties.fontName ~= nil and Properties.fontName or Group.fontName 
	
	local Charset = V.Charsets[fontName]
	if Charset and Charset.charSpacing ~= nil then Group.charSpacing = Charset.charSpacing end
	if Charset and Charset.lineSpacing ~= nil then Group.lineSpacing = Charset.lineSpacing end
...
local PDrawChars = function(Group)
	...
	local c, n, i, j, Img, len, x, y, w,h, yy, Char, Temp, spaceBefore, charSpacing, xadvance
	local ox = 0
	local oy = 0
	...
	for j = Group.startLine, Group.endLine do
		...
		y   		= oy + (linesDrawn-1) * (Charset.lineHeight + Group.lineSpacing*Charset.scale)
		x   		= ox
		...
		for i = 1, len do
			...
			if Char == nil then 
				print("!!! TextFX._DrawChars(): CHAR '"..c.."' NOT INCLUDED IN CHARSET '"..Group.fontName.."'!")
			else
				xadvance = Char.xadvance ~= nil and Char.xadvance*Charset.scale + Group.charSpacing*Charset.scale or Char.width + Group.charSpacing*Charset.scale
				-- IS NEW LINE?
				if n ~= 32 then
					if Charset.isVector then
						if Group.shadowOffX ~= 0 or Group.shadowOffY ~= 0 then
							...
							Temp:setFillColor( 0,0,0, Group.shadowAlpha )
							Img:insert(Temp)
							Temp = display.newText( c, 0, yy, Charset.name, Group.fontSize * Charset.scale )
							-- APPLY COLOR
							if Group.Color ~= nil then 
								if #Group.Color == 9 then
									Temp:setFillColor( { type='gradient', color1={Group.Color[1],Group.Color[2],Group.Color[3],Group.Color[4]}, color2={Group.Color[5],Group.Color[6],Group.Color[7],Group.Color[8]}, direction=Group.Color[9] } )
								else
									Temp:setFillColor( Group.Color[1],Group.Color[2],Group.Color[3],Group.Color[4] ) 
								end
							end
							Img:insert(Temp)
						else
							...
							if Group.Color ~= nil then 
								if #Group.Color == 9 then
									Img:setFillColor( { type='gradient', color1={Group.Color[1],Group.Color[2],Group.Color[3],Group.Color[4]}, color2={Group.Color[5],Group.Color[6],Group.Color[7],Group.Color[8]}, direction=Group.Color[9] } )
								else
									Img:setFillColor( Group.Color[1],Group.Color[2],Group.Color[3],Group.Color[4] ) 
								end
							end
						end
					else
						Img = V.newSprite(Charset.Set, Char.frame)
					        -- APPLY COLOR
					        if Group.Color ~= nil then 
					        	if #Group.Color == 9 then
						        	Img:setFillColor( { type='gradient', color1={Group.Color[1],Group.Color[2],Group.Color[3],Group.Color[4]}, color2={Group.Color[5],Group.Color[6],Group.Color[7],Group.Color[8]}, direction=Group.Color[9] } ) 
					...
					--
					Img.y           = y + (Char.offY * Charset.scale)
					--Img.y         = y + (Group.charBaseLine * Char.height*.5)

					-- CHAR BASELINE (0 = CENTER, 1 = TOP, -1 = BOTTOM)
    ...
    if Group.Scrollbar ~= nil then
		...
		Scrollbar.x, Scrollbar.y = Group[2]:contentToLocal(x, Group[2].contentBounds.yMin-4)
...
local PDoDeform = function(Group)
	...
	if Deform.type == V.DEFORM_SHAKE then
		...
		for j = 1, Group[2].numChildren do
			...
			-- FLIP CHARS?
			if Deform.flipCharsX == true then Group[2][j].xScale = -Group[2][j].xScale end
			if Deform.flipCharsY == true then Group[2][j].yScale = -Group[2][j].yScale end
		end
	--------------------------------------------------------
...
local PSetWrappedTextData = function(Group)
	...
	-- GET MAX HEIGHT
	Group.totalHeight = numLines * Charset.lineHeight + (numLines-1) * Group.lineSpacing*Charset.scale
